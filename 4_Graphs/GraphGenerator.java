...
public class GraphGenerator {
   private static final class Edge implements Comparable<Edge> {                             
    ...
    // Returns a random simple graph on V vertices, with an edge between any
    // two vertices with probability p. This is sometimes referred to as the
    // Erdos-Renyi random graph model.
    public static Graph simple(int V, double p) {
        if (p < 0.0 || p > 1.0)
            throw new IllegalArgumentException("Probability must be between"
                + "0 and 1");
        Graph G = new Graph(V);
        for (int v = 0; v < V; v++) {
            for (int w = v + 1; w < V; w++) {
    ...
    // Returns the compulate graph on V vertices.
    ...
    // Returns a random simple bipartite graph on V1 and V2 vertices with
    // E edges.
    ...
    public static Graph bipartite(int V1, int V2, double p) {
        if (p < 0.0 || p > 1.0)
            throw new IllegalArgumentException("Probability must be between 0"
                + " and 1");
        int[] vertices = new int[V1 + V2];
        for (int i = 0; i < V1 + V2; i++)
            vertices[i] = i;
    ...
    public static Graph eulerianCycle(int V, int E) {
        if (E <= 0)
            throw new IllegalArgumentException("An Eulerian cycle must have"
                + " at least one edge");
        if (V <= 0)
            throw new IllegalArgumentException("An Eulerian cycle must have"
                + " at least one vertex");
    ...
    public static Graph eulerianPath(int V, int E) {
        ...
        if (V <= 0)
            throw new IllegalArgumentException("An Eulerian path must be at "
                + "least one vertex");
        Graph G = new Graph(V);
    ...
    public static Graph wheel(int V) {
        if (V <= 1)
            throw new IllegalArgumentException("Number of vertices must be "
                + "at least 2");
        Graph G = new Graph(V);
        int[] vertices = new int[V];
        for (int i = 0; i < V; i++)
    ...
    public static Graph star(int V) {
        if (V <= 0)
            throw new IllegalArgumentException("Number of vertices must be at "
                + "least 1");
        Graph G = new Graph(V);
        int[] vertices = new int[V];
        for (int i = 0; i < V; i++)
    ...
    public static Graph regular(int V, int k) {
        if (V * k % 2 != 0)
            throw new IllegalArgumentException("Number of vertices * k must be"
                + " even");
        Graph G = new Graph(V);
    ...
    // Returns a uniform random tree on V vertices.
    // This algorithm uses a Prufer sequence and takes time 
    // proportional to VlogV.
    public static Graph tree(int V) {
        ...
        // degree of vertex v = 1 + number of times it appears 
        // in Prufer sequence
