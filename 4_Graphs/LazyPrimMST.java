...
public class LazyPrimMST {
    ...
    // check optimality conditions (takes time propotional to EVlgV)
    private boolean check(EdgeWeightedGraph G) {
        ...
        // check that if it is a spanning forest
        for (Edge e : G.edges()) {
            int v = e.either(), w = e.other(v);
            if (!uf.connected(v, w)) {
        ...
        // check that if it is a spanning foest
        for (Edge e : G.edges()) {
            int v = e.either(), w = e.other(v);
            if (!uf.connected(v, w)) {
                System.err.println("Not a spanning forest");
