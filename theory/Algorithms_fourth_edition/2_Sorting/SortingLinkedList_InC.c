...
/*
    ...
    on linked list, without requiring the O(N) extra space it needs
    when you do it on arrays.

    This code can handle singly and doubly linked, and
    circular and linear lists too. For any serious application,
    you'll probably want to remove the conditions on 'is_circular'
...
/*
    ...
    head of the list. (It has to, because the head will not generally
    be the same element after the sort.) So unlike sorting an array,
    where you can do
                    sort(myarray);
    you now have to
...
element *listsort(element *list, int is_circular, int is_double) {
    ...
    while (1) {
        ...
        nmerges = 0;    // count number of mrges we do in this pass
    ...
    for (is_circular = 0; is_circular < 2; is_circular++) {
        for (is_double = 0; is_double < 2; is_double++) {
            for (i = 0; i < sizeof(order)/sizeof(*order); i++) {
                printf("is_circular = %d, is_double = %d, i = %d\n", is_circular, is_double, i);
                int *ord = order[i];
                head = &k[ord[0]];
                for (j = 0; j < n; j++) {
                    if (j == n - 1)
