...
public class DigraphGenerator {
    ...
    //   Returns a random simple digraph on V vertices, with an edge between any
    //   two vertices with probability p. This is sometimes referred to as the 
    //   Erdos-Renyi random digraph model.
    //   This implementations takes time proportional to V^2 (even if p is small). 
    ...
    // The DAG returned is not chosen uniformly at random amony all such DAGs.
    public static Digraph rootedInDAG(int V, int E) {
        ...
        // one edge pointing from each vertex, other than the root = vertex[V - 1]
        for (int v = 0; v < V - 1; v++) {
